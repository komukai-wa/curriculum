...
// ③ 「USER_RNAME」の値のみ等しい場合。
 ...
 else if(name.equals(USER_NAME)){System.out.println(CONST_MSG_ERROR_PASS);}
// ④ 「USER_PASSWORD」の値のみ等しい場合。
       //定数を使用して「 名前に誤りがあります。 」と出力してください。
       if(name.equals(USER_NAME));
       else if (pass.equals(USER_PASSWORD)){System.out.println(CONST_MSG_ERROR_NAME);
       }
       // ⑤ 「USER_NAME」も「USER_PASSWORD」の値も間違っていた場合。 
       //定数を使用して「 入力情報に誤りがあります。 」と出力してください。
       if(!name.equals(USER_NAME)&&!pass.equals(USER_PASSWORD)){System.out.println(CONST_MSG_ERROR_INPUT);}
